...
export class TestStepsDAO {  
  ...
  async create(args: CreateTestStep): Promise<TestStepEntity> {
    ...
    const testStepsRow = (
      await this.db.insert(testEntitySteps).values(args).returning()
    ).at(0)!;
  ...
  async findById(id: number): Promise<TestStepEntity | undefined> {
    const row = await this.db
      .select()
      .from(testEntitySteps)
      .where(eq(testEntitySteps.id, id))
      .then((rows) => rows[0]);
  ...
  async findByTestId(testId: number): Promise<TestStepEntity[]> {
    return await this.db
      .select()
      .from(testEntitySteps)
      .where(eq(testEntitySteps.testId, testId))
      .orderBy(testEntitySteps.startedTimestamp, testEntitySteps.finishedTimestamp)
  ...
  async patch(args: PatchTestStep): Promise<TestStepEntity> {
    return await this.db.transaction(async (tx) => {
      const row = await tx
        .select()
        .from(testEntitySteps)
        .where(eq(testEntitySteps.id, args.id))
        .then((rows) => rows[0]);
...
export type TestStepsTable = typeof testEntitySteps;
