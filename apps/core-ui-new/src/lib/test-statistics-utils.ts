...
export const ENTITY_TYPES: ("test" | "beforeTest" | "afterTest")[] = ["test", "beforeTest", "afterTest"];

// Utilities
export const getStatusBackground = (color?: string | null): React.CSSProperties => {
 ...
 */
export const filterStatsByEntityType = (
  testEntityStats: TestEntityCount[]
): Record<string, TestEntityCount[]> => {
  return {
    test: testEntityStats.filter(stat => stat.entityType === "test"),
    beforeTest: testEntityStats.filter(stat => stat.entityType === "beforeTest"),
    afterTest: testEntityStats.filter(stat => stat.entityType === "afterTest")
  };
};
...
export const organizeStatsByEntityType = (
  filteredStats: Record<string, TestEntityCount[]>,
  statuses: Status[],
  statusGroups: StatusGroup[]
): Record<string, GroupedStatusData[]> => {
  return {
    test: organizeByStatusGroups(filteredStats.test, statuses, statusGroups),
    beforeTest: organizeByStatusGroups(filteredStats.beforeTest, statuses, statusGroups),
    afterTest: organizeByStatusGroups(filteredStats.afterTest, statuses, statusGroups)
  };
};
