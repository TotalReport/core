...
import {
  ENTITY_TYPE_LABELS,
  type GroupedStatusData,
} from "@/lib/test-statistics-utils.js";
import { StatusGroupSummary } from "./status-group-summary.js";
import { StatusGroupDetail } from "./status-group-detail.js";
          ...
          <ul className="space-y-2">
            {organizedStats.map((groupData) => (
              <StatusGroupDetail
                ...
                groupData={groupData}
                isExpanded={
                  !!expandedGroups[`${entityType}-${groupData.group.id}`]
                }
                onToggle={() =>
                  onGroupToggle(`${entityType}-${groupData.group.id}`)
                }
              />
            ))}
          </ul>
